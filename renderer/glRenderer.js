...
constructor(c,v){this.g=c.getContext("webgl")||c.getContext("experimental-webgl");
if(!this.g){alert("WebGL not supported");throw new Error("WebGL not supported");}
this.v=v;this.i();}
i(){const g=this.g;
...
draw(cs){const g=this.g,f=([x,y])=>[(x/g.canvas.width)*2-1,1-(y/g.canvas.height)*2];
const p=cs.flatMap(f);g.bindBuffer(g.ARRAY_BUFFER,this.b);
