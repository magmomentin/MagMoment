...
draw(cs){
const g=this.gl;
const f=([x,y])=>[(x/g.canvas.width)*2-1,1-(y/g.canvas.height)*2];
const p=cs.flatMap(f);
g.bindBuffer(g.ARRAY_BUFFER,this.b);
g.bufferData(g.ARRAY_BUFFER,new Float32Array(p),g.STATIC_DRAW);
const l=g.getAttribLocation(this.p,"a");
g.enableVertexAttribArray(l);
g.vertexAttribPointer(l,2,g.FLOAT,false,0,0);
g.bindTexture(g.TEXTURE_2D,this.t);
g.texImage2D(g.TEXTURE_2D,0,g.RGBA,g.RGBA,g.UNSIGNED_BYTE,this.video);
